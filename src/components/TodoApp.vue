<template>
  <div class="container">
    <h2 class="text-center mt-5">My Vue todo's App</h2>
    <div class="d-flex">
      <input
        type="text"
        v-model="task"
        ...
        class="form-control"
      />
      <button @click="saveTask" class="btn btn-success rounded-0">save</button>
    </div>
      ...
      <tbody>
        <tr v-for="(task, index) in tasks" :key="index">
          ...
          <td style="width: 120px">
            ...
            'text-success' : task.status === 'finished'}">{{ firstCharUpper(task.status) }}</span>
            </td>
